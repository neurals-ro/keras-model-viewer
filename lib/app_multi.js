...
$(function(){
  ...
  if (!json){
    drawModels("drawing", keras_ex1)
    return
  }
  $.getJSON( json, function( data ) {
    drawModels("drawing", data)
  });
})

function drawModels(elementId, mjson){
  for (var i = 0; i < rows; i++) {
    for (var j = 0; j < cols; j++) {
      $("#"+elementId).append("<div style='width:"+100/cols+"%;' id='draw_"+i+"_"+j+"'></div>");
      drawModel("draw_"+i+"_"+j, mjson)
    }
    $("#"+elementId).append("<br>");
  }
}
